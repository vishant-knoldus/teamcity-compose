...
open class ScheduleTrigger : Trigger {
    constructor(init: ScheduleTrigger.() -> Unit = {}, base: ScheduleTrigger? = null): super(base = base as Trigger?) {
    ...
    sealed class SchedulingPolicy(value: String? = null): CompoundParam(value) {
    ...
    sealed class TriggerBuild(value: String? = null): CompoundParam(value) {
    ...
    /**
     * Whether the build should be triggered with [clean checkout](https://www.jetbrains.com/help/teamcity/?Clean+Checkout)
     * @see enforceCleanCheckoutForDependencies
     */
    var enforceCleanCheckout by booleanParameter(trueValue = "true", falseValue = "")

    /**
     * When the enforceCleanCheckout is set to true specifies whether the [clean checkout](https://www.jetbrains.com/help/teamcity/?Clean+Checkout)
     * should also be applied to all dependencies.
     * @see enforceCleanCheckout
     */
    var enforceCleanCheckoutForDependencies by booleanParameter(trueValue = "true", falseValue = "")
 ...
 * @see ScheduleTrigger
 */
fun Triggers.schedule(base: ScheduleTrigger? = null, init: ScheduleTrigger.() -> Unit = {}) {
    trigger(ScheduleTrigger(init, base))
}
