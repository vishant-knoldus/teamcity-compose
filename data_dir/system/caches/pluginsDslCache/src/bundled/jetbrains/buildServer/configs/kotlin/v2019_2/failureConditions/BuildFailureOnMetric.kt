...
open class BuildFailureOnMetric() : FailureCondition() {
    ...
    constructor(init: BuildFailureOnMetric.() -> Unit): this() {
        init()
    }
    ...
    sealed class CompareTo(value: String? = null): CompoundParam<CompareTo>(value) {
        ...
        class Build() : CompareTo("true") {
            ...
            sealed class BuildRule(value: String? = null): CompoundParam<BuildRule>(value) {
                abstract fun validate(consumer: ErrorConsumer)
                ...
                class BuildWithNumber() : BuildRule("buildNumber") {
                    ...
                    override fun validate(consumer: ErrorConsumer) {
                        if (number == null && !hasParam("buildNumberPattern")) {
                            consumer.consumePropertyError("compareTo.buildRule.number", "mandatory 'compareTo.buildRule.number' property is not specified")
                        }
                    }
                }
                ...
                class BuildWithTag() : BuildRule("buildTag") {
                    ...
                    override fun validate(consumer: ErrorConsumer) {
                        if (tag == null && !hasParam("buildTag")) {
                            consumer.consumePropertyError("compareTo.buildRule.tag", "mandatory 'compareTo.buildRule.tag' property is not specified")
                        }
                    }
                }
            }
    ...
    override fun validate(consumer: ErrorConsumer) {
        ...
        if (metric == null && !hasParam("metricKey")) {
            consumer.consumePropertyError("metric", "mandatory 'metric' property is not specified")
        }
        compareTo?.validate(consumer)
    }
}
...
fun FailureConditions.failOnMetricChange(init: BuildFailureOnMetric.() -> Unit): BuildFailureOnMetric {
    val result = BuildFailureOnMetric(init)
