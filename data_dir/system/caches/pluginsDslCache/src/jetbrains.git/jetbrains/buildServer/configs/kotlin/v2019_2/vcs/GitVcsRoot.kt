...
open class GitVcsRoot() : VcsRoot() {
    ...
    constructor(init: GitVcsRoot.() -> Unit): this() {
        init()
    }
    ...
    sealed class AuthMethod(value: String? = null): CompoundParam<AuthMethod>(value) {
        ...
        class UploadedKey() : AuthMethod("TEAMCITY_SSH_KEY") {
            ...
            override fun validate(consumer: ErrorConsumer) {
                if (uploadedKey == null && !hasParam("teamcitySshKey")) {
                    consumer.consumePropertyError("authMethod.uploadedKey", "mandatory 'authMethod.uploadedKey' property is not specified")
                }
            }
        }
        ...
        class CustomPrivateKey() : AuthMethod("PRIVATE_KEY_FILE") {
            ...
            override fun validate(consumer: ErrorConsumer) {
                if (customKeyPath == null && !hasParam("privateKeyPath")) {
                    consumer.consumePropertyError("authMethod.customKeyPath", "mandatory 'authMethod.customKeyPath' property is not specified")
                }
            }
        }
    }
